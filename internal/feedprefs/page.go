...
<style>
:root{color-scheme:light dark;font:16px system-ui,sans-serif}body{max-width:680px;margin:3rem auto;padding:0 1.2rem;line-height:1.5}h1{margin-bottom:.3rem}p{color:light-dark(#52525b,#b8b8c2)}fieldset{border:0;padding:0}label{display:block;margin-top:1.3rem;font-weight:600}input[type=text],input[type=number]{box-sizing:border-box;width:100%;padding:.65rem;border:1px solid #888;border-radius:6px;font:inherit}#branches{border:1px solid #888;border-radius:6px;padding:.3rem .8rem .8rem;max-height:18rem;overflow:auto}.branch-heading{font-weight:600;margin-top:1.3rem}small{display:block;color:light-dark(#52525b,#b8b8c2);margin:.3rem 0}.check{font-weight:400;margin-top:.8rem}.check input{margin-right:.5rem}button{font:inherit;padding:.7rem 1rem;border:1px solid #888;border-radius:6px;cursor:pointer;margin:1.5rem .5rem 0 0}button[type=submit]{background:#275a44;color:white;border-color:#275a44}#status{min-height:1.5em}a{color:inherit}
...
<div id="branch-heading" class="branch-heading">Branches</div><small id="branch-help">Check the branches you want to see in your catalog. Uncheck a branch to hide its books.</small><div id="branches" role="group" aria-labelledby="branch-heading" aria-describedby="branch-help"></div>
...
function show(p){
 savedIncludes=[...(p.include_branches||[])];
 const included=new Set(savedIncludes),excluded=new Set(p.exclude_branches||[]),options=[...branches];
 for(const missing of new Set([...included,...excluded]))if(!options.some(b=>b.id===missing))options.push({id:missing,name:'Previously selected branch',subdomain:'unavailable'});
 el('branches').replaceChildren();
 for(const b of options){
  const label=document.createElement('label'),checkbox=document.createElement('input');
  label.className='check';checkbox.type='checkbox';checkbox.value=b.id;checkbox.checked=(!included.size||included.has(b.id))&&!excluded.has(b.id);
  label.append(checkbox,document.createTextNode(b.name+' ('+b.subdomain+')'+(b.online?'':' — offline')));el('branches').append(label);
 }
